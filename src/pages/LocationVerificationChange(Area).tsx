...
import ReferenceVerificationDocument from "../components/documents/ReferenceVerificationDocument";
...
export default function LocationVerificationChange({
  ...
  const hasUploaded = Boolean(profile?.locationVerificationChangeArea);
  const status = profile?.locationVerificationChangeAreaStatus || '';
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      formData.append("locationVerificationChangeArea", locationFile);
      ...
      updateData("locationPreview", null);
      updateData("locationFile", null);
    } catch (err: any) {
      const errorMessage = err.response?.data?.message || "Failed to upload location verification";
    ...
    if (status === 'PENDING') {
      return (
        <div className="flex items-center p-2 sm:p-4 mb-4 sm:mb-6 bg-gradient-to-r from-yellow-50 to-yellow-100 text-yellow-800 rounded-lg border-l-4 border-yellow-400 shadow-md animate-fade-in">
          ...
          <span className="font-medium">Verification Pending</span>: Awaiting administrator approval for location verification.
        </div>
      );
      ...
      return (
        <div className="flex items-center p-2 sm:p-4 mb-4 sm:mb-6 bg-gradient-to-r from-red-50 to-red-100 text-red-800 rounded-lg border-l-4 border-red-400 shadow-md animate-fade-in">
          ...
          <span className="font-medium">Verification Rejected</span>: The submitted location verification was not approved.
        </div>
      );
      ...
      return (
        <div className="flex items-center p-2 sm:p-4 mb-4 sm:mb-6 bg-gradient-to-r from-green-50 to-green-100 text-green-800 rounded-lg border-l-4 border-green-400 shadow-md animate-fade-in">
          ...
          <span className="font-medium">Verification Approved</span>: The location verification has been successfully approved.
        </div>
      );
    }
    ...
    <div className="w-full max-w-md sm:max-w-lg md:max-w-xl mx-auto p-4 sm:p-8 bg-gradient-to-br from-white to-gray-50 shadow-xl rounded-xl transition-all duration-300">
      <form onSubmit={handleSubmit}>
        <div className="flex flex-col items-center">
          {!hasUploaded ? (
            <>
              <label className="block text-sm font-semibold text-gray-800 mb-2">Upload Location Verification</label>
              <input
                type="file"
            ...
            <>
              <img
                src={profile?.locationVerificationChangeArea?.url || ''}
                alt="Uploaded Location Verification"
                className="mb-4 sm:mb-6 w-full max-h-[300px] rounded-lg shadow-md object-contain"
              />
              {renderStatusMessage()}
            </>
        ...
        {!isApproved && (
          <div className="flex items-center justify-center p-4 mb-4 bg-gradient-to-r from-blue-50 to-indigo-100 text-blue-800 rounded-lg border border-blue-200 shadow-md">
            ...
            <div className="text-center">
              ...
              <div className="text-xl font-bold text-blue-600 mt-2">{fees?.locationVerificationFee} INR</div>
            </div>
          </div>
        )}
        {
