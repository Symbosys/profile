...
export default function EmailVerification({
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (!otpSent) {
        ...
        updateData("otp", otp);
        updateData("profile", response.data.data.profile);
        nextStep();
      }
    } catch (err: any) {
      const errorMessage =
  ...
  return (
    <div className="w-full max-w-md sm:max-w-lg mx-auto p-4 sm:p-8 bg-gradient-to-br from-white to-gray-50 shadow-xl rounded-xl transition-all duration-300">
      {/* Messages */}
      {loading && (
        <div className="flex items-center justify-center p-2 sm:p-4 mb-4 sm:mb-6 bg-gradient-to-r from-blue-50 to-blue-100 text-blue-800 rounded-lg shadow-md animate-fade-in">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4 sm:gap-6 w-full">
        ...
        <div className="flex w-full items-center gap-2 sm:gap-4">
          ...
          <input
            ...
            className="flex-1 border border-gray-300 rounded-lg p-3 sm:p-4 focus:ring-2 focus:ring-teal-500  focus:border-teal-500 disabled:bg-gray-100 disabled:cursor-not-allowed transition-all duration-300"
            required
          />
          {!otpSent && (
            <button
              type="submit"
              disabled={isFormDisabled}
              className={`px-4 sm:px-8 py-2 sm:py-3 rounded-lg text-white font-semibold text-base sm:text-lg shadow-lg transition-all duration-300 ${
                isFormDisabled
        ...
        <div className="flex flex-col sm:flex-row justify-between w-full mt-4 sm:mt-8 gap-2 sm:gap-4">
          {prevStep && (
            <button
              type="button"
              onClick={prevStep}
              disabled={isLoading}
              className={`w-full sm:flex-1 px-4 sm:px-6 py-2 sm:py-3 rounded-lg text-slate-800 font-semibold text-base sm:text-lg shadow-md transition-all duration-300 ${
                isLoading
                  ? "bg-gray-400 cursor-not-allowed"
        ...
        { (
          <button
            ...
            className="w-full mt-2 sm:mt-4 px-4 sm:px-6 py-2 sm:py-3 rounded-lg bg-red-600 hover:bg-red-700 text-white font-semibold text-base sm:text-lg shadow-md transition-all duration-300 active:scale-95"
          >
            Use Another Email
          </button>
        )}
