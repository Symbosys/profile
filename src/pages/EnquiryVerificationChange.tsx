...
export default function EnquiryVerificationChange({
  ...
  const status = profile?.enquiryVerificationChangeStatus || '';
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      formData.append("enquiryVerificationChange", enquiryFile);
      ...
      const errorMessage = err.response?.data?.message || "Failed to upload enquiry verification";
    ...
    if (status === 'PENDING') {
      return (
        <div className="flex items-center p-4 mb-6 bg-gradient-to-r from-yellow-50 to-yellow-100 text-yellow-800 rounded-lg border-l-4 border-yellow-400 shadow-md animate-fade-in">
          ...
          <span className="font-medium">Verification Pending</span>: Awaiting administrator approval for enquiry verification.
        </div>
      );
      ...
      return (
        <div className="flex items-center p-4 mb-6 bg-gradient-to-r from-red-50 to-red-100 text-red-800 rounded-lg border-l-4 border-red-400 shadow-md animate-fade-in">
          ...
          <span className="font-medium">Verification Rejected</span>: The submitted enquiry verification was not approved.
        </div>
      );
      ...
      return (
        <div className="flex items-center p-4 mb-6 bg-gradient-to-r from-green-50 to-green-100 text-green-800 rounded-lg border-l-4 border-green-400 shadow-md animate-fade-in">
          ...
          <span className="font-medium">Verification Approved</span>: The enquiry verification has been successfully approved.
        </div>
      );
    }
    ...
    <div className="max-w-xl mx-auto p-8 bg-gradient-to-br from-white to-gray-50 shadow-xl rounded-xl transition-all duration-300">
      <form onSubmit={handleSubmit}>
        <div className="flex flex-col items-center">
          {!hasUploaded ? (
            <>
              <label className="block text-sm font-semibold text-gray-800 mb-2">Upload Enquiry Verification</label>
              <input
                type="file"
